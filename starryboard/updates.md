...
* サウンドの再生：シーンに対してサウンドファイルを指定して音を鳴らせるように（MVなどBGM合わせで描くのを想定した機能）
* インポート：シナリオライティングソフトの出力形式への対応
* エクスポート：映像編集ソフトの対応フォーマットファイルへの対応
* リストページに絵コンテスタイル以外にグリッド表示やテキスト重視の表示を追加
