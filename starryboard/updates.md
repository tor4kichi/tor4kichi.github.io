...
* v0.9 BGM対応、カメラとレイヤーのアニメーション
* v0.10 範囲選択、ブラシ、カラーパレットの改修
...
* 画像ファイルの取り込みに対応しました
  ...
  * 追加先タグと各カットの表示時間を指定できます
  * 画像サイズがキャンバスサイズよりも大きい場合は、キャンバスが埋まるギリギリの大きさに縮小リサイズして取り込みます  

【エクスポート】

* エクスポート画面をすっきりとした表示に
  * 対応しているエクスポート方法をすべて表示する形に変更しました
  * 追加する場合はエクスポートアイテムを右クリックして「コピーを作成」を選びます
  * 「出力後にエクスプローラーを表示して出力アイテムを確認させる動作」を削除しました
    * エクスポートボタンの部分が完了後は別ボタンを表示（エクスプローラーで開く、クリップボードにコピーなど）
  * デフォルトの出力先指定の表示を削除しました
    * エクスポート時に個別に出力先を選択する形に変更しています
* 画像・動画を出力するエクスポートにおいて
  * 表示タグを指定できるように
  * 進捗状態を表示するように
  * 出力先選択中にもエクスポート処理を進めるように
* 編集用動画のエクスポートに対応しました
  * OpenTimelineIO（.otio）およびFCPXML(.fcpxml) の出力に対応
  * タグごとにトラックを分けて出力
* CSVのエクスポートに対応しました
  * すべてのカットをフラットな構造で出力します
* Jsonのエクスポートに対応しました
  * エピソードの階層構造（シーン・カット・フレーム）を保って出力します
* 絵コンテエクスポート時に
  * 画像形式（.jpgまたは.png）で出力できるように
  * 出力画像の見映えを調整して違和感を減らすように
  * 出力画像のプレビュー機能を追加

<!-- 

* 台本エクスポートの出力形式がPDFからdocx（Wordファイル）に変わりました
  * カット情報をアフレコ台本風にレイアウトして出力します
  * [台本テンプレートを差し替えによる任意レイアウトでの出力にも対応](/features/export-scriptbook-template)
...
* エピソード作成時の画面を整理しました
* エピソード作成時に設定のコピー元となるエピソードを選択しやすくしました
* エピソード作成時のキャンバスサイズ入力方法として画面比率プリセットからも指定できるように

【その他の変更点】

* 単一キーフレームに同一タグを指すレイヤーが複数指定できないように
* 「ストローク変更時のサムネイル画像更新」をデフォルトで有効とするように
* エピソード一覧画面でエクスポートボタンが無効となるように修正
* 先頭シーンは削除できないようボタンを無効化するよう修正
...
【キャンバス】
* エピソードを開き直したときにキャンバス内のボタンなどが表示されない問題を解消
* キャンバスのカラーパレットを調整しました
  * カラーコードの入力やRGBをそれぞれ調整できるタブを追加
  * カラーパレット上の各カラーを右クリックor長押しすると色選択ポップアップが表示されます
* カラーパレットのタグ別カラーとレイヤー一覧のタグ別レイヤーにおけるタグ選択が同期するように
* エピソード作成時にデフォルトで追加するタグのテーマカラーとタグ別カラーの色を調整しました
* ブラシサイズをタグごとに保持するように
* カメラ範囲の枠線の色が設定の「キャンバスとカット」タブから変更できるように
* タグ選択ごとにペン先切り替えが安定するように
* オニオンスキン表示時にCPU/GPUのリソースが過剰に消費されていた問題を解消
* オニオンスキンのオプション表示にオニオンスキン範囲選択された区間のカット番号表示に反映されるように
* オニオンスキンの色置換の有効無効を切り替えたら即座にキャンバスに反映されるように

【カットリスト】
* キーフレームの接続を切り替えたときにタイムが意図しない値に変化してしまう問題を解消
* オニオンスキンの範囲選択を開始した時にリスト全体が選択されていた問題を解消
  * 接続されたキーフレームすべて、または手前１カットが選択されます

【プレビュー】
* プレビューを開いた直後にESCキーなどショートカットキーによって閉じられかった問題を解消

### v0.7.0 

大きな変更点のまとめ
* レイヤー管理がフレーム単位からカット単位に
  * カット内フレームが同じレイヤーを参照しやすい形になりました。（以前までは別フレームのレイヤーを参照する手間が多くかかっていました）
* キャンバスの枠外も描けるように
  * キャンバスのサイズを無限にすることでカット毎のキャンバスサイズの指定が不要に。キーフレームごとにカメラ範囲を指定することで動画出力に対応します
* 画面レイアウトのバリエーションが増えました
  * カットリストとキャンバスを左右分割する以外に、以前もあったキャンバスのみ表示を再登場させた上で、新たにプレビューを「カットリストと一緒に表示」「キャンバスと一緒に表示」とする表示方法に対応しています
* （そのほかの細かい変更点が多数あります）
...
各レイヤー画像がカットに登録された上で、キーフレームごとにレイヤーを選択する形になります。

キーフレームごとにレイヤーを選択する際にもう一つ制約があります。タグ１つにつきレイヤー１枚という制約です。

例えば、カットに４つのタグを表示するように指定したら、そのタグ一つずつに一枚のレイヤーを指定します。キーフレームごとにタグにどのレイヤーを表示するのか選択することになるため、前キーフレームと同じレイヤーを表示することも、異なるレイヤーを表示することもできます。

新しいキーフレームを追加すると新しいレイヤーが自動的に追加される、という挙動が基本です。

背景のように次キーフレームでもレイヤーを流用したい場合には、キャンバスのタグ別レイヤーから前キーフレームに指定されたレイヤーを指定することで同じレイヤーを複数のキーフレームで扱うことができます。

複数キーフレームで同一レイヤーを指定する操作をショートカットするための設定があります。タグの設定から「ゼロタイム方向のキーフレームのレイヤーを継承する」を有効にすることで、キーフレーム追加時に前キーフレームと同じレイヤーを自動的に指定するようになります。デフォルトは「無効」に設定されています。


#### キャンバスの枠外も描けるように

キャンバスのサイズ指定が無くなって無制限に描き込めるようになりました。

エピソード作成時に指定したキャンバスサイズを元にキーフレームごとにカメラ範囲が置かれます。カメラ範囲を動かすことでプレビューや動画出力の際に表示する場所を指定できます。

カメラ範囲はキャンバス上部のカメラアイコンのボタンから編集できます。

編集用キャンバスの表示位置を選択中キーフレームのカメラ範囲に戻したい場合はキャンバス左下の虫眼鏡アイコンを押してください。

#### 画面レイアウトのバリエーションが増えました

* キャンバス表示をカットリストとの左右分割及び上下分割、カットリストを表示しない全体表示の３種類に対応しました
* プレビュー再生の表示方法として「キャンバスと一緒に表示」「カットリストと一緒に表示」に対応しました
  * プレビュー画面の表示先ボタンから選択できます
* 設定とエクスポートの画面をすぐ閉じられるよう大きくポップアップする表示にしました
* 全画面表示をタイトルバーから切替できるように

#### そのほかの変更点

【キャンバス】
* カメラアニメーションの機能はカメラ範囲を指定する形に機能制限しています
  * 無限キャンバス上のどの範囲を写すのかを決める事は出来ますが、プレビュー・動画エクスポートした動画上ではスライド移動（PAN）はしません
  * 今後の更新で改めてカメラアニメーション、レイヤーアニメーションへの対応を検討しています
* キャンバスUIにおけるレイヤー管理ボタンはキーフレームにおけるタグ別の表示レイヤー管理ボタンになりました
* キャンバスにおける範囲選択と消しゴムのレイヤー貫通編集を有効にする切替ボタンを追加しました
* キャンバスのズームイン・ズームアウトがポインター位置を中心に動作するように
* キャンバスの画像読み込みやキーフレームのリスト表示用の画像読み込みを効率化しました
* ベクター画像のラスター画像への変換操作は、レイヤー一覧からタグ別表示レイヤーの切り替えを表示して、レイヤーアイテムを右クリックまたは長押しして表示されるメニューからアクセスする形に変更しました
* キャンバスの下部にカット番号やタイムなどを表示する設定項目を「アプリ設定」の「キャンバスにテキストを表示する」に移動しました
* 「タッチで描く」設定を有効にした際の描き込みモードとキャンバス操作モード（平行移動や拡大縮小）を切り替えるためのボタンを追加しました
  * UNDO/REDOボタンの右側にある「ひし形アイコンのボタン」からキャンバス操作モードに切り替えられます  
* キャンバスの範囲選択に「カット」操作を追加しました
* ペースト操作がUNDO/REDOに対応しました
* ペースト時に画面の中心になるように

【カットリスト】
* カット番号と秒数の表示を大きくしました
* キーフレームの秒数がゼロの場合に秒数部分の背景を薄い赤色表示するように
* 編集時のカット追加ボタンをリストアイテムの左端に変更
  * 同時にキーフレーム追加ボタンを削除しました。カット追加後に接続状態を変更してください
* カットリストを大きい画面で表示した際によりウィンドウにフィットした表示になるよう調整しました
* キーフレームのアイテムメニューにシーン区切りの切替ボタンを追加しました
* 上下分割表示時に比率変更グリップUIの表示を上下移動できそうな表示に修正しました
* プレビューやオニオンスキンを選択する際にシーンリストからシーンごとにキーフレーム選択できるように
* シーン作成時やシーンのアイテムメニューから「カットの一括追加」を指定できるように
* 検索がスムーズにできるよう表示を整えました

【プレビュー】
* プレビュー再生がスムーズに。内部的に読み込みと画像キャッシュ、表示の更新処理を効率化しました
* プレビューの操作方法を変更しました
  * 画面クリックで再生と一時停止が切り替わります  
  * 操作パネルを下側に集約しました
  * 左右にスワイプすると再生位置を前後に移動できます
  * 上下にスワイプすると操作パネルを開閉できます
  * 画面下部をクリックすると操作パネルを開閉できます
  * 画面を右クリックすると操作パネルを開閉できます
* プレビューする際にタイム指定が０秒のキーフレームはカット番号を画面右上に表示します

【エクスポート】
* エクスポートUIの表示を簡略化しました
* 連番画像エクスポートの画像の高さ指定の変更が反映されるように

【設定】
* カットの初期秒数指定をエピソードファイルから削除し、一括追加時に秒数を指定できるように
  * カットやキーフレームを一つだけ追加する場合は常にゼロ秒です
* ショートカット設定の「前を検索」「次を検索」の項目が逆に入れ替わっていた問題を修正しました
...
シーンの管理UIを変更しました。「シーン区切りに指定」が削除され、今後はシーンがカットを保有する形になります。

* シーン一覧を開くボタンはリストページの左上に移動しました
* リストページ右上の「リスト編集」からカット追加、カット削除、カット並び替えの操作ができます

#### キャンバスUIのレイアウトの変更点

リストページにキャンバスUIを組み込む形に変更しました。カット一覧とキャンバスUIが分割表示されます。

* 画面サイズの比率によって左右分割と上下分割が自動で切り替わります
  * 手動で変更したい場合は、アプリ設定から「キャンバスの分割表示方向を自動で変える」をOFFにすると、切り替えスイッチがキャンバスUIの右上に表示されるようになります
* キャンバスUIにあった一部のボタンを削除しました。
  * リストページと重複するため整理として削除しています
  * ダイアログやアクションのテキスト変更ボタン、タイムの変更ボタン、プレビューボタン、エクスポートボタンが無くなってます
* オニオンスキンの範囲選択はリストページのカット一覧から選択するようになりました
  * 選択中は一時的にキャンバスUIが閉じられます

#### キャンバスUIのブラシ（旧名：ドロワー）の変更点

* 描写関係のキャンバスアクションをキャンバス右側にまとめて配置しています
  * カラーパレットとタグ別カラーパレットを一つのカラーボタンに統合しました
  * ブラシサイズを指定するスライダーを追加しました
  * 従来のドロワーを縦に並べて表示しています
* キャンバスアクション（描写、消しゴム、範囲選択、変形）の選択ボタンのレイアウトを調整しました
  * 配置が右手用になってますが、今後左手用レイアウトにも対応予定です
* キャンバスアクションの一番右側にある３つの点ボタンにあったタグ別カラーパレットなどの表示切替項目を削除しました
* ブラシ設定がエピソードではなくアプリ側に記録されるように変更しました
  * 合わせてブラシ設定のバックアップ、リストアにも対応しています
* ブラシ設定にサイズ変更に関する項目を追加しました
  * ブラシごとに「最大サイズ」「最小サイズ」「サイズ変更値」を指定できます

#### その他の変更点

* レイヤー強調表示のボタンをキャンバス左下に移動しました
  * 切り替え対象を選択中レイヤーの強調表示だけに絞りました
  * 以前は選択中レイヤーの保有タグを強調表示できていましたが、扱いづらくなっていたので無くしました
  * ボタンを右クリック（長押しタップ）すると「非対象レイヤーの不透明度」を変更できます
* キャンバスサイズ変更時にエピソード指定のキャンバスサイズを元にしたガイド線を表示します
  * キャンバスサイズ変更画面の左下にガイド線に対する「スナップ」「分割レベル」を操作できます
* ショートカット設定の表示を更新しました
  * キーボードショートカットとジェスチャーショートカットを別々に設定するように


### ~~v0.5.2~~ v0.5.3

プレビュー画面のレイアウトを整理してわかりやすくしました。特に再生範囲の指定を簡単に。また、キャンバスページのカット番号や秒数、アクションやダイアログなどの表示をサイド固定できるように。

プレビューの変更点
* コントロールUIを上部の開閉式からプレイヤーにオーバーラップする形に
  * 上側にウィンドウ管理、下側に再生管理のコマンドを配置しています
  * 画面のどこかを押すとコントロールUIの表示・非表示を切り替え
* 再生範囲の選択方法がシーン選択＋シーン内から範囲選択の２段階選択に
  * コントロールUI下側、シークバーの左下にある再生カット範囲が表示されたボタンからシーン選択、再生範囲選択できます
  * シーン選択には「全てのシーン」も含まれます。基本的にはシーンごとや接続カットごとにチェックし、加えて通し再生も出来る形にしました
* 再生管理表示の右下にある設定ボタンにカット番号の表示・非表示の切替を設定を追加
* 表示タグの保存先をエピソードファイルからアプリローカルのエピソード毎設定に
* プレビュー再生で表示しているカットを編集する操作を追加
  * 現在カット番号を押して「このカットを編集」を選択します


キャンバスページの変更点
* オニオンスキン（前後カットの一時参照表示）の表示切替ボタンを画面左下に移動しました
* 一時的な参照表示を設定するボタン（一時参照ボタン）をキャンバスページの左下に追加しました
  * ウィンドウキャプチャと外部画像参照を一時参照として追加できます
  * 一時参照の表示はエピソードファイルに保存されず、またプレビュー・カットサムネイル・エクスポートにも影響しません
* カット一覧の下にあるカット情報とプレビュー・カット追加・エクスポートの表示を画面右側に表示するオプションを追加
  * キャンバス上側にあるボタン類の右側の３点ボタンから「カット番号などをサイドに表示」を選択すると表示を切り替えられます
* 「タッチで描写する」が有効な時だけキャンバス変形の補助ボタンを表示するように
* 消しゴムと範囲選択におけるレイヤー貫通の対象選択方法を「選択中レイヤー」「タグ」「全レイヤー」の３種類から選ぶ形に
* レイヤー強調表示を消しゴム・範囲選択以外に描写中も利用できるように


その他の変更点
* アセットレイヤー一覧にアイテムが無い時の文言のローカライズ対応
* カットにアセットレイヤーを追加削除した時、接続カット切り替え時にカットのサムネイル画像とプレビューを更新するように
* プレビュー画面がエスケープキーで閉じられなかった問題を修正
...
* 接続カットについて
  * 親カットに含まれるアセットレイヤーは子カットにも同期・反映されます
  * （v0.5.2以降修正予定） 既に接続された状態のカットにアセットレイヤーを追加しても他接続カットと同期されない問題があります
