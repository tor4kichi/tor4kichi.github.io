...
* v0.6 シーン管理の強化とサウンド再生
* v0.7 カメラとレイヤーのアニメーション
...
大きな変更点のまとめ
* カットとレイヤーの管理方法が変わりました。時間軸としてのキーフレームと、キーフレームごとのタグ別レイヤーを統合しました
* カット毎のキャンバスサイズ指定が無くなりました。無限キャンバスにすることでキャンバスサイズの操作が不要になり、キーフレームごとにカメラ範囲を指定することで動画出力に対応します
* アプリの画面レイアウトの選択肢が増えました。カットリストとキャンバスを左右分割する以外に、以前もあったキャンバスのみ表示を再登場させた上で、新たにプレビューを「カットリストと一緒に表示」「キャンバスと一緒に表示」とする表示方法に対応しています

#### キーフレームとタグ別レイヤー管理

カットにおけるレイヤー管理方法が大きく変わりました。

カット内フレーム間でレイヤーが常に共有されるようになり、各フレームがタグごとにレイヤー表示のON/OFFを選択する形になります。

レイヤーの管理方法
* v0.6まで：各フレームにレイヤーを自由に追加でき、レイヤーに任意にタグ付けができた
* v0.7から：カットに各タグ一つずつのレイヤーが自動で追加され、キーフレームごとにタグに表示すべきレイヤーを追加or選択できる

この変更により共有レイヤーが無くなります
* v0.6まで：複数フレームで同一のレイヤー画像を表示するには共有レイヤー機能を経由していた
* v0.7から：カットにキーフレームを追加すると前キーフレームと同内容のレイヤーを引き継ぐ。変更したいタグだけ別のレイヤー画像を追加or選択して切り替えられる

#### 無限キャンバス

* キャンバスの描きこみ範囲の制限がなくなりました
  * これに伴いキャンバスサイズを変更するUIが無くなりました
* キーフレームごとにカメラ範囲を指定することで出力時に表示する位置と大きさを決めます
  * カメラ範囲はキャンバス上部のカメラアイコンのボタンから編集可能です
...
* カメラアニメーションの機能はカメラ範囲を指定する形に機能制限しています
  * 無限キャンバス上のどの範囲を写すのかを決める事は出来ますが、プレビュー・動画エクスポートした動画上ではスライド移動はしません
...
* マウスホイールによるキャンバスのズームイン・ズームアウトがマウス位置を中心に動作するように
