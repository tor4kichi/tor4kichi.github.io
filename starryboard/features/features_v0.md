...
* 事前に決定できるものは決めてしまって、絵コンテ作業中の決断コストを減らそう
  * ペン（色・形）、レイヤー構造、プレビュー対象とするレイヤーカテゴリなどをシーン設定として予め決めておく（後から変更できるが）

## アプリウィンドウについて

* Starryboardアプリは複数起動できます。ただし複数ウィンドウから一つのシーンファイルは開けません。
* １シーンファイルに対する複数ウィンドウ表示は今後対応予定です（特にプレビューウィンドウ）
...
* 画像：外部画像を参照します。アプリセッション中のみ画像をメモリに保持します。シーンファイルには保存されません。
