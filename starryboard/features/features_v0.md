...
* 事前に決定できるものは決めてしまって、絵コンテ作業中の決断コストを減らそう

## アプリウィンドウについて

* Starryboardアプリは複数起動できます。ただし複数ウィンドウから一つのシーンファイルは開けません。


## シーン管理ページ

* シーンファイルを管理します。フォルダを追加して作品シリーズごとに仕分けできます。
* シーン名の変更・削除は右クリック（タッチ操作の場合はホールドタップ）で表示されるメニューから操作できます
* 並べ替えはページ上部の右から３番目のボタンからできます。手動の並べ替えは非対応です。

![シーン管理ページのスクリーンショット](SceneListPage001.png)

![シーン管理ページでシーンアイテムを右クリックした際のスクリーンショット](SceneListPage002.png)

## カット一覧ページ

* シーンに内包された全てのカットを表示
* セリフなどのテキストと秒数はココで編集できます
...
* シーンに含まれるカット全てが表示されます
* 青い枠線で囲まれたカットが選択中のカットです。選択中カットがキャンバスから編集できます。
* カットリストのうち赤い下線のあるカットアイテムはプレビュー範囲を示します。カットリストをスクロールすると自動でスクロール位置に応じたカットアイテムがプレビュー範囲として設定されます。
* カットアイテムの間に表示されている「吹き出しアイコン」と「クリップ」のアイコンは
  ...
  * クリップアイコンはカットの接続状態を示します。
    * A. レイヤーカテゴリに共有レイヤーが設定されている場合に編集・表示対象の画像が共有されます。背景として同じを使いたい場合にレイヤーカテゴリの指定とカット接続を組み合わせて利用します。
...
シーン共通のキャンバスサイズは設定画面から確認できます。

#### リファレンス

* ウィンドウキャプチャ：他のアプリウィンドウをStarryboardアプリ内にキャプチャ表示します。３Ｄソフトやブラウザ、写真や動画など自由に表示できます。再取り込みすることなくウィンドウの表示に合わせて自動で更新されて便利ですが、多少GPU使用率が増加します。

* 画像：外部画像を参照します。アプリセッション中のみ画像をメモリに保持し、シーンファイルには保存されません。
...
MemoやMainと並んでいる部分が「レイヤーカテゴリ」、左側のプラスマークのボタンに挟まれた default とあるのが「レイヤーコンテナ」です。
...
レイヤーカテゴリやレイヤーコンテナ指定するロールは、設定ページから編集できます（アプリウィンドウの右上にある歯車のアイコンからアクセスできます）（レイヤーカテゴリの設定ページを開くと次の画像のように表示されます）。これらの設定はシーンファイルに保存されます。
