...
* [サムネイル画像とキャッシュ](#サムネイル画像とキャッシュ)
  * [ファイルシステム管理のサムネイル生成を優先](#ファイルシステム管理のサムネイル生成を優先)
  * [ファイル名に「cover」が含まれるファイルをサムネイル画像として優先的に使用します](#ファイル名に「cover」が含まれるファイルをサムネイル画像として優先的に使用します)
  * [フォルダのサムネイル画像を変更する](#フォルダのサムネイル画像を変更する)
  * [ファイルのサムネイル画像は変更できません](#ファイルのサムネイル画像は変更できません)
* [別ウィンドウにビューアを表示](#別ウィンドウにビューアを表示)
...
アプリ設定の画像ビューアの項目から画像を表示する際のフィルター処理方法を指定できます。

#### 補間処理の種類

画像を縮小表示する際、複数ピクセルを１ピクセルに圧縮しますが、その処理方法を選べます。HighQualityCubicはより滑らかに見えます。MultiSampleLinearは十分な滑らかさと元の画像の印象が残るバランスがよいです。

以下、Microsot公式が提供している開発ドキュメントより機械翻訳した説明です

* HighQualityCubic    
  * 可変サイズの高品質な3次カーネルを使用して、変換行列に縮小が含まれている場合に画像の事前縮小を行います。その後、最終出力には3次補間モードを使用します。

<!-- * Anisotropic    
  * 異方性フィルタリングを使用して、画像の変換後の形状に応じてパターンをサンプリングします。 -->
* MultiSampleLinear 
  * 1ピクセル内で4つの線形サンプリングを行い、エッジのアンチエイリアシングを効果的に行います。このモードは、ピクセル数の少ない画像をわずかに縮小する場合に適しています。
<!-- * Cubic    
  * 16サンプルの3次カーネルを使用して補間を行います。このモードは処理時間が最もかかりますが、より高品質な画像を出力します。 -->
* Linear    
  * 4点のサンプリングと線形補間を使用します。NearestNeighborと比較すると、このオプションは処理速度が遅くなりますが、より高品質な画像が出力されます。
<!-- * NearestNeighbor
  * 最も近い1点をサンプリングし、その正確な色を使用します。このオプションは処理速度が最も速いですが、出力される画像の品質は最も低くなります。 -->

翻訳元： https://microsoft.github.io/Win2D/WinUI3/html/T_Microsoft_Graphics_Canvas_CanvasImageInterpolation.htm

#### シャープネス

エッジ（色の境界部分）の鮮明に表示する度合いを指定できます。 0%は何も処理せずボカした表示になり、100%にすると境界がより鮮明に表示されます。
...
動画ビューアの上部、ウィンドウバーにもプレイヤー表示を制御するボタンがあります。左から「動画コンテンツのお気に入り切替」「映像効果編集」「表示変形編集」「回転（時計回り」」「左右反転」「全画面表示」があります。

映像効果編集を選択すると、再生中の動画の色を調整できます。明るさ・コントラスト・彩度が変更できます。

> 映像効果を使用すると僅かにCPU・GPUの利用率が増加しますが、映像効果を「リセット」すると負荷のない動作に戻ります。
...
> この項目では、ツバメビューアにおけるサムネイル画像の取り扱いについて説明します

フォルダ一覧、画像一覧を表示する際の各表示アイテムの画像はキャッシュすることで高速に表示しています。

アプリ設定からキャッシュの生成と保存の種類について設定できます → アプリ設定の「サムネイル画像のキャッシュ生成モード」

### ファイルシステム管理のサムネイル生成を優先

アプリ既定の動作では、サムネイル画像はWindowsOSのファイルシステムが管理している画像を参照してアプリ上で表示しています。多くの画像ファイルと動画ファイルは基本的にファイルシステム経由でサムネイル画像を取得しています。

こうすることでファイルシステムとアプリ側とでキャッシュのストレージ領域を二重に占有せずに済みます。

ファイルシステムが対応していない、zipアーカイブやepubファイル、一部動画ファイルなどはアプリ側でサムネイル画像を生成しキャッシュに保存して表示します。

### ファイル名に「cover」が含まれるファイルをサムネイル画像として優先的に使用します

フォルダアイテムの初回表示時、サムネイル画像とするファイルを選択する条件は優先度順に「名前にcoverがあるファイル」「複数ある場合はより新しい方を優先する」「無い場合は新しい順の先頭ファイル」となっています。

zipやepubでも同様で、coverを含むファイル名を持つエントリーを優先します。cover画像が無ければアーカイブ内部構造の並び順の先頭で表示できる画像を探索します。

### フォルダのサムネイル画像を変更する

フォルダについてはサムネイル画像を変更できます。既にcover.jpgがある場合、上書きを許可しない場合は変更をキャンセルします。

### ファイルのサムネイル画像は変更できません

zip/epub/動画ファイルのサムネイル画像の変更は v2.6 時点で非対応です。

フォルダに展開してcover画像を変更して再度アーカイブ化する。または動画の場合は動画編集ソフト等でサムネイル画像を変更して再保存することで変更は可能です。

外部でサムネイル画像を変更した場合、アプリ内にサムネイル画像を反映させるにはフォルダアイテムのメニューから「サムネイル画像の再読み込み」を選択してください。

## 別ウィンドウにビューアを表示する

アプリ設定のビューア設定から「別ウィンドウにビューアを表示する」を有効にすると、画像・小説・動画の各ビューアは別ウィンドウを開いてそこに表示されるようになります。通常は一つの別ウィンドウを使いまわしてビューア表示されます。

ビューアを複数表示したい場合は、ビューア表示可能なアイテムのアイテムメニューから「新しいウィンドウで開く」を選択すると、常に新しいウィンドウを作成してコンテンツを表示できます。

複数の別ウィンドウが表示されている時の新しくビューアを表示するウィンドウの選択条件は、より古いタイミングで開いたウィンドウを利用して表示します。

別ウィンドウに関する動作の制約があります

* アプリがクラッシュするとすべてのウィンドウが同時に終了します → すべてのウィンドウが同一プロセスで動作するため
